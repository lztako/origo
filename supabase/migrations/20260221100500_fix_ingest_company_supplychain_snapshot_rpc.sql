-- Fix ambiguity in RPC and tighten execute privileges.
...
begin
  ...
  if p_replace_snapshot then
    delete from public.company_supplychain as cs
    where cs.snapshot_id = btrim(p_snapshot_id);
  end if;
...
revoke all on function public.ingest_company_supplychain_snapshot(text, jsonb, boolean) from anon;
revoke all on function public.ingest_company_supplychain_snapshot(text, jsonb, boolean) from authenticated;
